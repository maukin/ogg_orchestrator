...
class PrepareSourceExecutor(ABC):
    @abstractmethod
    def execute(
        self,
...
class DryRunPrepareExecutor(PrepareSourceExecutor):
    def execute(
        self,
        commands: list[SourcePrepareCommand],
        artifacts_dir: str | Path,
        ...
        (out_dir / "source_prepare_commands.sql").write_text(
        ...
        return ExecutorResult(
            ...
            skipped_count=0,
            raw_output="Dry-run prepare source completed.",
            error_code=None,
            error_message=None,
            http_status=None,
            request_artifact=str(out_dir / "source_prepare_commands.sql"),
            response_artifact=str(out_dir / "source_prepare_commands.json"),
        )


class FileOnlyPrepareExecutor(PrepareSourceExecutor):
    def execute(
        self,
        commands: list[SourcePrepareCommand],
        artifacts_dir: str | Path,
        ...
        return ExecutorResult(
            ...
            executed_count=len(commands),
            skipped_count=0,
            raw_output="File-only prepare source completed.",
            error_code=None,
            error_message=None,
            http_status=None,
            request_artifact=str(out_dir / "source_prepare_commands.sql"),
